...
const REACTIONS=[
  ...
  /* 18개까지 늘린 뒤에도 "그래도 적다"는 지적이 이어졌다. 이번엔 물어봤다 — 목표가
     ...
     그래서 이번엔 반응 '갈래'를 새로 만들지 않고, 이미 검증된 여섯 갈래(화합·연소/분해/
     금속+산/금속의 반응성(치환)/탄산염+산) 안에서 같은 모양을 다른 물질로 반복한다 —
...
const ELEMENTS=[
  ...
  {z:84,sym:'Po',name:'폴로늄',cat:'post',group:16,period:6,desc:'폴로늄은 <strong>금속결합</strong>을 하는 전이 후 금속으로, <strong>Po²⁺</strong>·<strong>Po⁴⁺</strong> 이온이 되는 매우 강한 방사성 원소다. 금속과 비금속을 가르는 대각선 위에 있어 <strong>준금속</strong>으로 칠한 표도 많지만, 온도가 오르면 전기 전도도가 오히려 <strong>낮아지는</strong> 금속다운 성질을 보인다. 원소 가운데 유일하게 <strong>단순 입방</strong> 결정 구조를 이룬다. 마리 퀴리와 피에르 퀴리가 발견해 조국 폴란드의 이름을 땄고, 강한 알파선을 내놓아 정전기 제거 장치의 방사선원으로 쓰인다.'},
...
/* 표의 색과 범례, 그리고 원소 상세의 「분류」가 전부 이 목록 하나를 본다.
   여기 없는 cat이 있으면 그 원소는 범례에 없는 색으로 칠해지고 상세의 분류가 빈칸이 된다 —
   109번 이후 8종이 실제로 그랬다. 표에 쓰는 cat은 빠짐없이 여기 있어야 한다. */
const PT_CATEGORIES=[
  ...
  /* 초중원소 일부는 몇 개 만들어 보지도 못하고 곧바로 붕괴해 화학적 성질을 확정하지 못했다.
     「모른다」가 사실이므로 그렇게 적는다 — 아무 분류에나 밀어 넣으면 없는 사실을 가르친다.
     "109번부터 전부"가 아니다. 실제로 성질을 확인한 둘은 분류가 붙어 있다:
     코페르니슘(112)은 기체 화학 실험으로 12족 금속답게 행동함이 확인돼 전이 금속,
     플레로븀(114)은 같은 방식으로 14족 금속 성질이 확인돼 전이 후 금속이다.
     나머지 Mt·Ds·Rg·Nh·Mc·Lv·Ts·Og 여덟만 미확인이다. */
  ['unknown','성질 미확인']
];
...
/* ── 단원자 이온의 한글 이름 ──
   양이온은 「원소 이름 + 이온」이라 규칙으로 만들어도 되지만(나트륨 이온·칼슘 이온),
   음이온은 「-화 이온」이라 이름이 바뀐다 — Cl⁻은 염소 이온이 아니라 **염화 이온**이다.
   규칙으로 만들지 않고 적어 두는 이유: 「소로 끝나면 소를 화로 바꾼다」로 하면 염소→염화,
   산소→산화, 질소→질화까지는 맞는데 수소만 수화가 되어 틀린다(H⁻은 수소화 이온).
   예외 하나 때문에 규칙이 조용히 거짓말을 하느니 목록으로 둔다.
   ION_FORMING이 만들 수 있는 음이온 전부를 담는다. */
const ANION_KO={ H:'수소화', N:'질화', P:'인화', O:'산화', S:'황화', F:'플루오린화', Cl:'염화' };
function ionNameKo(sym, dir){
  const e=ELEMENTS.find(x=>x.sym===sym);
  return (dir==='gain' ? (ANION_KO[sym]||e.name) : e.name)+' 이온';
}
...
const PRECIPITATES=[
  ...
  {a:'Pb^2+',b:'I^-',    f:'PbI2',   name:'아이오딘화 납(Ⅱ)', color:'노란색'},
  {a:'Cu^2+',b:'S^2-',   f:'CuS',    name:'황화 구리(Ⅱ)',    color:'검은색'},
  {a:'Pb^2+',b:'S^2-',   f:'PbS',    name:'황화 납(Ⅱ)',      color:'검은색'},
...
/* 앙금 색을 글자로만 쓰면 「흰색/노란색/검은색」이 다 같은 글자색이라 눈에 안 들어온다.
   실제로 눈으로 구분하는 게 앙금 문제의 핵심이므로 색을 같이 보여 준다.
   검은색은 다크 테마 배경에 묻히므로 스와치에 테두리를 준다(css의 .ppt-sw).
   색만으로 뜻을 전하지는 않는다 — 글자에 이미 색 이름이 있고 스와치는 거드는 역할이다. */
const PRECIP_COLORS = { '흰색':'#F1F5F9', '노란색':'#FACC15', '검은색':'#0B1220' };
...
const ION_KO={
  ...
  'Ca^2+':'칼슘 이온',   'Ba^2+':'바륨 이온',   'Pb^2+':'납(Ⅱ) 이온',
