...
function covalentDiagramHTML(b, opts){
  ...
  /* 원자를 어디에 놓느냐가 곧 분자 모양이라, 아무 데나 두면 틀린 그림이 된다.
     ...
     104.5°에 맞췄고, 사면체(CH₄)는 평면으로 펴서 고르게 벌린 표현이다. */
  let angles;
  if(n === 1)      angles = [0];                       /* 이원자 분자 */
  else if(n === 2) angles = cLone > 0 ? [142, 38] : [180, 0];  /* 물 굽은형 104° / 이산화탄소 직선형 */
  /* 암모니아(삼각뿔). 예전에는 120°씩 고르게 벌렸는데, 그러면 두 가지가 어긋났다.
     하나, 그 그림은 평면삼각형이다 — NH₃는 삼각뿔이라 세 H가 한쪽으로 기울어 있다.
     둘, 고르게 벌리면 「빈 쪽」이 없어서 비공유 전자쌍이 H와 H 사이에 끼어 앉고,
     N 둘레에 네 방향이 거의 같은 간격으로 늘어서 암모니아가 4개짜리로 보였다.
     세 H를 한쪽으로 모아 부채꼴(70°씩)로 두고 비공유 전자쌍을 그 반대쪽에 놓는다 —
     삼각뿔을 옆에서 본 모양이고, 아래 gapDir 계산이 이 배치에서 정확히 반대쪽(위)을
     가리킨다. 70°는 이웃한 H 껍질이 안 닿는 가장 좁은 각 근처다(test/diagram.mjs 가 잰다). */
  else if(n === 3) angles = [20, 90, 160];
  ...
  /* 리간드가 셋 이상인 분자(NH₃·CH₄)는 입체 구조다. 평면에 옮겨 그리는 이상 각도는
     실제와 다를 수밖에 없는데, 그림만 보면 그 사실을 알 길이 없다 — 한 줄로 밝힌다.
     예전에는 이 사실이 코드 주석에만 적혀 있었다. 학생은 주석을 읽지 않는다. */
  const flat = n >= 3 ? ' 실제로는 입체 구조여서, 평면에 옮긴 이 그림의 각도는 실제 각도와 다르다.' : '';
  const exp = (order
    ? `${uniq} — 전자쌍 <b>${pairs0}쌍</b>을 공유하므로 <b>${BOND_ORDER_NAME[pairs0]}</b>이다.
       공유하는 전자쌍이 늘수록 두 원자가 더 세게 붙잡혀 결합이 짧고 강해진다.`
    : `${uniq}. 두 껍질이 겹친 자리에 찍힌 것이 그 전자쌍이다.`) + flat + key;
  return `<div class="dia-wrap">
