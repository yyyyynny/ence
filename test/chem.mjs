...
/* ── 폴로늄(84): 전이 후 금속 ──
   한동안 준금속으로 뒀다가 문헌을 찾아보고 바꿨다. 준금속에는 널리 받아들여지는 정의가
   없어서(영어 위키백과 「Metalloid」: "No widely accepted definition of a metalloid
   exists") 목록이 저자마다 다른데, Vernon 2013(J. Chem. Educ. 90(12) 1703–1707)이
   준금속 목록 194종을 조사한 결과 B·Si·Ge·As·Sb·Te 는 86~99%인 반면 Po·At 는
   40~49%로 절반에 못 미친다(Se 24%).
   성질로 보면 한쪽으로 기운다 — 온도가 오르면 전기 전도도가 **낮아지는** 금속형 거동을
   보이고(준금속·반도체는 반대다), 원소 가운데 유일하게 단순 입방 구조를 이룬다.
   영국 왕립화학회는 "It is classed as a metal", 브리태니커는 "places polonium among
   the metals rather than the metalloids or nonmetals" 라고 적는다.
   준금속으로 칠하는 표가 흔한 것은 성질이 아니라 **대각선 위라는 자리** 때문이다.
   앱 안의 기준도 이것으로 통일된다 — 셀레늄(34)은 이미 좁은 기준(비금속)을 쓰고 있었다.
   교육과정 밖 원소이고 퀴즈 출제 범위(PT_QUIZ_ELEMENTS)에도 없어 채점에는 영향이 없다.
   (2022 개정 과학과 교육과정 본문에는 「준금속」·「비금속」이라는 말 자체가 0회다 —
    금속/비금속/준금속 3분류는 한국 교육과정의 것이 아니라 주기율표 색칠 관행이다.) */
REF_CAT[84]='post';
let en=0;
...
/* 값이 여럿이라 이름에 숫자가 필요한 금속. 납도 +2·+4 둘 다 흔하다(PbO / PbO₂) —
   앙금 쪽에만 남아 있던 「황화 납」·「아이오딘화 납」이 이 표에 Pb 가 없어서 통과했다. */
const VAR_METAL={Cu:'구리', Fe:'철', Pb:'납'};
